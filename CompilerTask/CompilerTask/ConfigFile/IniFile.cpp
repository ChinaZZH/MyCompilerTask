...
WriteInt : Writes an integer to the ini file
*========================================================================*/
void  CIniFile::WriteUInt(CCHR *pSection, CCHR *pKey, unsigned int Value)
{
	char Val[12]; /* 32bit maximum + sign + \0 */
	sprintf(Val, "%d", Value);
	WriteString(pSection, pKey, Val);
...
/*=========================================================================
ReadInt : Reads a unsigned integer from the ini file
*========================================================================*/
unsigned int CIniFile::ReadUInt(CCHR *pSection, CCHR *pKey, unsigned int Default)
{
	char Val[12] = { 0 };
	sprintf(Val, "%d", Default);
	CCHR* pValue = ReadString(pSection, pKey, Val);
	sscanf(pValue, "%d", &Default);
	return Default;
}
